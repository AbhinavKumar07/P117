...
def predict(text):
    ...
    padded_sequence = pad_sequences(sentence_sequence , maxlen = max_length , padding = padding_type , truncating = trunc_type)
    padded_testing = np.array(padded_sequence)

    predicted_class = np.argmax(model.predict(padded_testing) , axis = 1)
    print(predicted_class)

    for key , value in emo_code_url.items():
        if (value[0] == predicted_class):
            predicted_emo_url_img = value[1]
            predicted_emotion = key
    return predicted_emotion , predicted_emo_url_img
